...
function playBingo() {
  ...
  do {
    ...
    while (boardsToMark.some(board => isWinningBoard(board))) {
      let winningBoardIndex = boardsToMark.findIndex(board => isWinningBoard(board));
      boardsThatWon.push(boardsToMark.splice(winningBoardIndex, 1)[0]);
    }    
  
  } while (boardsToMark.length > 0);

  let lastWinning = boardsThatWon[ boardsThatWon.length - 1 ];
  let lastWinningSum = getSumOfBoard(lastWinning);

  console.log(lastWinningSum * lastCalledNum)
}
